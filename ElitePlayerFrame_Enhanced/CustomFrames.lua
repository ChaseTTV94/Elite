...
table.insert(C,function (addon)	-- Death Knight - Alliance	-- Function called with addon during initialisation
	return {	-- Custom frame object returned to addon for registration
		...
		"FF33FFFF",	-- Color when displayed in text output
		{
			...
			{(addon.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or addon.BRANCH == WOW_PROJECT_CLASSIC) and addon.SetPointOffset(-22,6) or addon.SetPointOffset(-23.5,7.5)},	-- Level text x,y offsets for SetPoint()
			{(addon.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or addon.BRANCH == WOW_PROJECT_CLASSIC) and addon.SetPointOffset(-1,0) or addon.SetPointOffset(-3,0)},	-- Rest icon x,y offsets for SetPoint()
...
table.insert(C,function (addon)	-- Death Knight - Horde
	return {
		...
		"FF33FFFF",
		{
			...
			{(addon.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or addon.BRANCH == WOW_PROJECT_CLASSIC) and addon.SetPointOffset(-24,0) or addon.SetPointOffset(-26,1.5)},
			{(addon.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or addon.BRANCH == WOW_PROJECT_CLASSIC) and addon.SetPointOffset(-1,0) or addon.SetPointOffset(-3,0)},
			function (addon)
...
table.insert(C,function (addon)	-- Demon Hunter
	return {
		...
		"FF99FF33",
		{
			...
			{(addon.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or addon.BRANCH == WOW_PROJECT_CLASSIC) and addon.SetPointOffset(-10,13.5) or addon.SetPointOffset(-12,14.5)},
			{(addon.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or addon.BRANCH == WOW_PROJECT_CLASSIC) and addon.SetPointOffset(1.5,0)},
			function (addon)
