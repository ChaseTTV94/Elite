		...
		if (stage == 1) then
			...
			self.RunOnce("InitialiseStage1Localisation",function ()
				...
				for k,v in pairs(maleClasses) do
					...
					self:AddClass(k,{k,maleClasses[k],femaleClasses[k]},self.GetHexColor(RAID_CLASS_COLORS[k]))	-- #WorkAround: Convert rgb to hex with rounding ourselves
				end
				---- [ Expansions ]
				self.EXPANSIONS = {}
				...
				self:AddTexture(L["Standard"],"Interface\\TargetingFrame\\UI-TargetingFrame.blp",{1,0.09375,0,0.78125},nil,(self.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or self.BRANCH == WOW_PROJECT_CLASSIC) and {self.SetPointOffset(0,-0.5)} or {self.SetPointOffset(0,0.5)})
				self:AddTexture(nil,"Interface\\TargetingFrame\\UI-TargetingFrame-Rare.blp",{1,0.09375,0,0.78125},nil,(self.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or self.BRANCH == WOW_PROJECT_CLASSIC) and {self.SetPointOffset(0,-0.5)} or {self.SetPointOffset(0,0.5)})
				self:AddTexture(nil,"Interface\\TargetingFrame\\UI-TargetingFrame-Rare-Elite.blp",{1,0.09375,0,0.78125},nil,self.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC and {self.SetPointOffset(0,-0.5)} or (self.BRANCH == WOW_PROJECT_CLASSIC and {self.SetPointOffset(2,-0.5)} or {self.SetPointOffset(0,0.5)}),self.BRANCH == WOW_PROJECT_CLASSIC and {self.SetPointOffset(1.5,0)})
				self:AddTexture(nil,"Interface\\TargetingFrame\\UI-TargetingFrame-Elite.blp",{1,0.09375,0,0.78125},nil,(self.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC or self.BRANCH == WOW_PROJECT_CLASSIC) and {self.SetPointOffset(0,-0.5)} or {self.SetPointOffset(0,0.5)})
			...
			if self.BRANCH ~= WOW_PROJECT_CLASSIC and self.BRANCH ~= WOW_PROJECT_BURNING_CRUSADE_CLASSIC then
...
function addon:GetExpansionLevel()
	...
	elseif self.BRANCH == WOW_PROJECT_BURNING_CRUSADE_CLASSIC then
		--return math.min(GetClassicExpansionLevel(),GetExpansionLevel(),GetAccountExpansionLevel())	-- #ToDo: Do we need to use the classic version too/only? Need to test in future expansion #Bug: Blizzard decided to ignore the GetAccountExpansionLevel API for BCC, and GetExpansionLevel() updating was wonky for launch
		return GetExpansionLevel()	-- #WorkAround: Just use GetExpansionLevel() directly until Blizzard improve the API and we can retest a launch in WLKC
	end
	return 0
end
